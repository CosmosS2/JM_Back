...
router.get("/userdata", async (req, res) => {
  try {
    ...
    if (!username) {
      return res
        .status(400)
        .json({ success: false, message: "El nombre de usuario es necesario" });
    }
...
router.post("/register", async (req, res) => {
  try {
    ...
    console.log("USUARIO EN BACK: ", nombre_usuario, password, id_rol);
    ...
    if (!usernameValidador.test(nombre_usuario)) {
      return res.status(400).json({
        success: false,
        message:
          "El nombre de usuario solo debe contener letras, números y/o guion medio, sin espacios.",
      });
    }
    ...
    const estado = 1;
    const query =
      "INSERT INTO usuario (nombre_usuario, contraseña, estado, id_rol) VALUES (?, ?, ?, ?)";
...
router.get("/listAll", async (req, res) => {
  try {
    const [results] = await db.query(
      "SELECT id, nombre_usuario, estado, id_rol FROM usuario"
    );
    if (results.length > 0) {
...
router.get("/listActive", async (req, res) => {
  try {
    const [results] = await db.query(
      "SELECT nombre_usuario, estado, id_rol FROM usuario where estado = 1"
    );
    if (results.length > 0) {
      return res.json({ success: true, userData: results });
    } else {
...
router.put("/update", async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, message: "ID del usuario es requerido" });
    }

    if (password) {
      const hashedPassword = await bcrypt.hash(password, 10);
      const query =
        "UPDATE usuario SET nombre_usuario = ?, contraseña = ?, id_rol = ?, estado = ? WHERE id = ?";
      ...
      const query =
        "UPDATE usuario SET nombre_usuario = ?, id_rol = ?, estado = ? WHERE id = ?";
